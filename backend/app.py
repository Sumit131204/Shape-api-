...
def remove_background(input_path, output_path, api_key=None):
    """Removes background from the input image and saves the output.
    
    If api_key is provided, uses the Remove.bg API, otherwise falls back to local rembg library.
    """
    try:
        print(f"[DEBUG] Starting background removal for {input_path}")
        if api_key:
            print(f"[DEBUG] Attempting to use Remove.bg API")
            # Use Remove.bg API
            ...
            if response.status_code == requests.codes.ok:
                print(f"[DEBUG] API request successful, saving result to {output_path}")
                with open(output_path, 'wb') as out:
                    out.write(response.content)
                return output_path
            else:
                print(f"[ERROR] Remove.bg API error: {response.status_code} {response.text}")
                # If API credit limit reached or other error, fall back to local rembg
                if response.status_code == 402:  # Payment Required
                    print("[DEBUG] API credit limit reached, falling back to local removal")
                else:
                    print("[DEBUG] API error, falling back to local background removal")
        else:
            print("[DEBUG] No API key provided, using local background removal")
        
        # Use local rembg library (original implementation)
        print(f"[DEBUG] Using local rembg library for {input_path}")
...
def detect_shape_color():
    ...
    try:
        ...
        try:
            ...
            # Remove background and save as PNG
            remove_background(filepath, bg_removed_path, api_key=app_settings.get('api_key'))
...
def detect_shape():
    ...
    try:
        ...
        try:
            remove_background(original_path, bg_removed_path, api_key=app_settings.get('api_key'))
...
def detect_size():
    ...
    try:
        ...
        try:
            remove_background(filepath, bg_removed_path, api_key=app_settings.get('api_key'))
            print(f"[DEBUG] Background removal completed successfully")
            if not os.path.exists(bg_removed_path):
                print(f"[ERROR] Background removed file doesn't exist after processing")
                return jsonify({'error': 'Background removal failed - output file not created'}), 500
        except Exception as bg_error:
            print(f"[ERROR] Background removal failed: {str(bg_error)}")
            return jsonify({'error': f'Background removal failed: {str(bg_error)}'}), 500
